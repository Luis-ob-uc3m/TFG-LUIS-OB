...
if __name__ == '__main__':
    ...
    #Se deben guardar todos los fit de las generaciones!
    filename = "OutputResults.json"
    result = json.load(open(filename))["Results"][0]
    fits = [generacion["Fits"]for generacion in result["ResultadoObtenido"]]
    fig, ax = plt.subplots()
    # Creating plot
    plt.boxplot(fits)

    plt.title("Evolución genereaciones")
    plt.ylabel("Rendimiento (€)")
    plt.xlabel("Generación")   # show plot
    plt.show()